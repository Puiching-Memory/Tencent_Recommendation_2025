...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=dataset.collate_fn, persistent_workers=False, pin_memory=True
    )
    valid_loader = DataLoader(
        valid_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=dataset.collate_fn, persistent_workers=False, pin_memory=True
    )
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            step_time = time.time() - data_load_start_time  # 包含数据加载时间的完整步骤时间
            global_step += 1
            epoch_steps += 1
            ...
            # 计算平均数据加载时间
            avg_data_load_time = np.mean(data_loading_times[-10:]) if data_loading_times else 0
            ...
            writer.add_scalar('Performance/avg_data_load_time', avg_data_load_time, global_step)
